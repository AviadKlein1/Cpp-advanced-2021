...
public:
	...
	int successor(T value) { return successor(Tree<T>::root, value); };
...
private:
	...
	int successor(typename Tree<T>::Node* current, T val);
...
void SearchTree<T>::remove(typename Tree<T>::Node* current, T val)
{
	...
	if (y != nullptr) y->parent = z->parent;
	typename Tree<T>::Node* p;
	if (z->parent != nullptr)p = z->parent;
	else (p = nullptr);
	if (z == p->left) p->left = y;
	else p->right = y;
	return;
...
template<class T>
int SearchTree<T>::successor(typename Tree<T>::Node* current, T val)
{
	if (!search(val)) {
		throw "no successor\n";
	}// no such node
	...
	if(max(current) == current_)throw "no successor\n";

	if (current_->right != nullptr) // if our node has right son
		return min(current_->right)->value;
...
typename Tree<T>::Node* SearchTree<T>::search_(typename Tree<T>::Node* current, T val)
{
	...
	if (current->value == val)
		return current;
	if (current->value < val)
		search_(current->right, val);
	else
		search_(current->left, val);
}
template<class T>
